...
describe('Test Scenario Fixtures', () => {
  ...
  test('should provide macOS development scenario', () => {
    ...
    const mockIntelGPUs = scenario.devices.filter((d) =>
...
describe('Fixture Utilities', () => {
  ...
  describe('Merge GPU Devices', () => {
    ...
    test('should handle empty arrays', () => {
      const result = fixtureUtils.mergeGPUDevices(
        [],
        [intelGPUFixtures.arcA770()],
        [],
      );
  ...
  describe('Filter Devices', () => {
    test('should filter devices by vendor', () => {
      ...
      const nvidiaDevices = fixtureUtils.filterDevicesByVendor(
        devices,
        'nvidia',
      );
    ...
    test('should filter OpenVINO compatible devices', () => {
      ...
      const compatibleDevices =
        fixtureUtils.filterOpenVINOCompatibleDevices(devices);

      expect(compatibleDevices).toHaveLength(2);
      expect(
        compatibleDevices.every((d) => d.capabilities.openvinoCompatible),
      ).toBe(true);
    });
  });
  ...
  describe('Performance Variance', () => {
    test('should add realistic performance variance', () => {
      ...
      const variedMetrics = fixtureUtils.addPerformanceVariance(
        baseMetrics,
        10,
      );
      ...
      const memoryUsageVariance =
        Math.abs(variedMetrics.memoryUsage - baseMetrics.memoryUsage) /
  ...
  describe('Custom Test Device', () => {
    test('should create custom test device with overrides', () => {
      ...
      expect(customDevice.id).toMatch(/^custom-\d+-[a-z0-9]+$/);
    });
    ...
    test('should generate unique custom device IDs', () => {
      ...
      expect(device1.id).toMatch(/^custom-\d+-[a-z0-9]+$/);
      expect(device2.id).toMatch(/^custom-\d+-[a-z0-9]+$/);
    });
